...
<%@ page language="java" import="java.util.*,com.book" pageEncoding="UTF-8"%>
<%@ page import="Order.ShoppingCart"%>
<html>
<head>
...
<body>
    ...
    {
        var form = document.forms[0];
        form.action = "order_edit";
        form.submit();
    }
    function Showorder()
    {
        var form = document.forms[0];
        form.action = "show_order";
        form.submit();
    }
    </script>
    <%
			...
			<div class="left_content">
				...
				<div class="feat_prod_box_details">
					<form action="addorder" method="post">
						<table class="cart_table" id="large" cellspacing="0">
							<tr class="cart_title">
								<td>书籍图片</td>
								<td>书籍编号</td>
								<td>书籍名称</td>
							...
							<%
								...
								List<ShoppingCart> pList = (List<ShoppingCart>) request
										.getSession().getAttribute("ShoppingCart");
								for (int i = 0; i < pList.size(); i++) {
									ShoppingCart sc = pList.get(i);
							%>
							<tr>
								<td><a href="bookInfo.jsp?bookID=<%=sc.getbookNumber()%>"><img src="<%=sc.getImagepath()%>" width="100"
									height="100" alt="" title="" border="0" class="cart_thumb" /></a>
								</td>
								<td><%=sc.getbookNumber()%></td>
								<td><%=sc.getorderBookName()%></td>
								<td><%=sc.getbookCount()%></td>
								<td><%=sc.getpayment()%></td>
								<td><%=sc.getpayment() * sc.getbookCount()%>
									<td><input type="checkbox" value=<%=sc.getbookNumber()%>
										name="bookNumber" />
								</td>
							</tr>
							<%
							...
							<%
								float allCost = 0;
								for (ShoppingCart sc : pList) {
									allCost += sc.getpayment() * sc.getbookCount();
								}
								String all_Cost = allCost + "";
								session.setAttribute("all_cost", all_Cost);
							%>
							<tr>
								<td colspan="4" class="cart_total"><span class="red">总价:</span>
								</td>
								<td><%=allCost%></td>
							</tr>
						</table>
						<%
							if (pList.size() > 0) {
						%>
						<form action="delete" method="post" width="100">
							<td><input type="submit" value="删除书籍" name="delete"
								onclick="Delete()" class="delete" />
							</td>
							<td><input type="submit" value="订单" name="order"
								onclick="Order()" class="order" />
							<td>
						</form>
						<%
							}
